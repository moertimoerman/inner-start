...
const BUILD_ROOT = path.join(ROOT, "public/audio/build/female-final");
...
function ensureTool(name) {
  ...
  if (result.status !== 0) {
    throw new Error(`${name} ontbreekt in PATH.`);
  }
}
...
function secondsFromProbe(filePath) {
  ...
  if (!Number.isFinite(n)) {
    throw new Error(`Kan duur niet lezen: ${filePath}`);
  }
  return n;
}
...
function buildClipPlan() {
  ...
  for (const theme of THEMES) {
    for (let i = 0; i < 10; i += 1) {
      ...
      clips.push({
        ...
        sourceFile: `FemaleNL3${capitalize(theme.key)}${n}.m4a`,
      });
      seq += 1;
    }
    for (let i = 0; i < 10; i += 1) {
      ...
      clips.push({
        ...
        theme: theme.key,
        silenceAfter: theme.silence,
        sourceFile: `FemaleNL1${capitalize(theme.key)}${n}.m4a`,
      });
      seq += 1;
    }
  }
  ...
  if (clips.length !== 100) {
    throw new Error(`Verwacht 100 clips, kreeg ${clips.length}.`);
  }
  return clips;
}
...
function verifySourceExists(clips) {
  for (const clip of clips) {
    ...
    if (!fs.existsSync(p)) {
      throw new Error(`Bronclip ontbreekt: ${p}`);
    }
  }
}
...
function verifyOutput(label, outputFile, clips, gaps, expectedTotalSeconds) {
  ...
  const pass = durationPass && silencedetectPass && midpointPass;
...
function main() {
  ...
  const fullListPath = path.join(LIST_DIR, "female-100-concat.txt");
  ...
  const debugListPath = path.join(LIST_DIR, "female-debug-5-concat.txt");
  writeConcatList(debugTimeline.concatItems, debugListPath);

  const debugWav = path.join(OUTPUT_DIR, "female-debug-first5-with-silence.wav");
  const debugM4a = path.join(OUTPUT_DIR, "female-debug-first5-with-silence.m4a");
  renderFromConcatList(debugListPath, debugWav);
  encodeWavToM4a(debugWav, debugM4a);

  const fullWav = path.join(OUTPUT_DIR, "female-full-100-with-themed-silence.wav");
  const fullM4a = path.join(OUTPUT_DIR, "female-full-100-with-themed-silence.m4a");
  ...
  saveJson(path.join(REPORT_DIR, "female-render-report.json"), report);
